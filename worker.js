...
async function getRestoreRollbackStatus(
  ...
  try {
    ...
    const safeControlResetRecorded =
      String(
    ...
    const retryAuthorizedAt =
      row.rollback_retry_authorized_at
        ? String(
            row.rollback_retry_authorized_at
          )
        : null;

    const retryAuthorizationExpiresAt =
      row.rollback_retry_authorization_expires_at
        ? String(
            row.rollback_retry_authorization_expires_at
          )
        : null;
    ...
    const retryAuthorizationExpiresMs =
      retryAuthorizationExpiresAt
        ? Date.parse(
            retryAuthorizationExpiresAt
          )
        : NaN;

    const retryAuthorizationActive =
      Boolean(
        safeControlResetRecorded &&
        retryAuthorizedAt &&
        retryAuthorizationExpiresAt &&
        Number.isFinite(
          retryAuthorizationExpiresMs
        ) &&
        retryAuthorizationExpiresMs >
          Date.now() &&
        retryAttempt === 0
    ...
    const retryAuthorizationExpired =
      Boolean(
        retryAuthorizedAt &&
        retryAuthorizationExpiresAt &&
        Number.isFinite(
      ...
      recovery: {
        ...
        actions: {
          ...
          retry_authorization: {
            ...
            authorized_by:
              normalizePositiveInteger(
                row.rollback_retry_authorized_by
              ) || null,
            expires_at:
              retryAuthorizationExpiresAt,
            active:
...
const ADMIN_RESTORE_ROLLBACK_SAFE_RESET_LIMIT = {
  maxRequests: 3,
  windowSeconds: 60 * 60,
  blockSeconds: 60 * 60,
};
...
const ROLLBACK_RETRY_AUTHORIZATION_TTL_MINUTES = 5;
    ...
    return {
      ...
      request_policy: {
        ...
        rollback_recovery_running_failed_reconcile_enabled:
          true,
        rollback_recovery_safe_control_reset_enabled:
          true,
...
// =========================
// ADMIN AUTHORIZE ONE EXPLICIT ROLLBACK RETRY
// Stage 2I-SR14F-D5B-2I
//
// Authorization only. This endpoint does NOT dispatch GitHub Actions,
// does NOT clear rollback_github_run_id, does NOT change rollback_status,
// and does NOT execute D1 Time Travel. It creates a single short-lived
// authorization marker only after a prior safe_control_reset and a fresh
// proof that the previous rollback run completed unsuccessfully before any
// destructive rollback stage started.
// =========================
Router.register(
  "POST",
  "/api/admin/restore/rollback/retry/authorize",
  async (ctx) => {
    ...
    const authorizeLimit =
      await SecurityRateLimit.consume({
        env: ctx.env,
        scope:
          "admin-restore-rollback-retry-authorize",
        key:
          `user:${admin.user_id}`,
        ...ADMIN_RESTORE_ROLLBACK_RETRY_AUTHORIZE_LIMIT,
      });
    ...
    const expectedConfirmationPhrase =
      `AUTHORIZE ROLLBACK RETRY ${restoreExecutionId}`;
    ...
    if (
      ...
      return {
        error:
          "rollback_retry_authorization_confirmation_phrase_incorrect",
        required_confirmation_phrase:
          expectedConfirmationPhrase,
      };
    }
    ...
    if (
      ...
      return {
        error:
          "rollback_retry_authorization_not_applicable",
        restore_status:
          journal.status || null,
        rollback_status:
          journal.rollback_status || null,
    ...
    if (
      ...
      return {
        error:
          "rollback_retry_history_already_initialized",
        retry_prohibited:
          true,
      };
    }
    ...
    if (
      ...
      return {
        error:
          "rollback_retry_authorization_already_active",
        expires_at:
          existingExpiresAt,
      };
    }
    ...
    if (
      ...
      return {
        error:
          "rollback_retry_authorization_progress_detected",
        destructive_progress_detected:
          destructiveProgressDetected,
        rollback_stage_started:
          anyRollbackStageStarted,
    ...
    if (githubLookup.error) {
      return {
        error:
          "rollback_retry_authorization_github_lookup_failed",
        github_error:
          githubLookup.error,
        retry_prohibited:
          true,
    ...
    if (
      ...
      return {
        error:
          "rollback_retry_authorization_github_run_not_found",
        retry_prohibited:
          true,
      };
    }

    const githubRunStatus =
      githubLookup.run.status
        ? String(
            githubLookup.run.status
          )
    ...
    if (
      ...
      return {
        error:
          "rollback_retry_authorization_previous_failure_not_proven",
        github_run_status:
          githubRunStatus,
        github_run_conclusion:
          githubRunConclusion,
    ...
    const expiresAt =
      new Date(
        ...
        ROLLBACK_RETRY_AUTHORIZATION_TTL_MINUTES *
          60 *
          1000
      );

    const authorizedAtIso =
    ...
    const authorizationResult =
      await ctx.env.OPS_DB.prepare(`
        ...
        WHERE restore_execution_id = ?
          ...
          AND rollback_verify_status IS NULL
          AND (
            rollback_retry_authorization_expires_at IS NULL
            OR rollback_retry_authorization_expires_at <= ?
          )
      `)
        .bind(
          authorizedAtIso,
    ...
    if (
      Number(
        authorizationResult?.meta?.changes ||
        0
      ) !== 1
    ) {
      return {
        error:
          "rollback_retry_authorization_failed",
        retry_prohibited:
          true,
      };
    }
    ...
    await SecurityAudit.recordSafe(
      ctx,
      {
        actorUserId:
          admin.user_id,
        action:
          "admin.restore_rollback_retry_authorize",
        targetType:
          "restore_execution",
        targetId:
          String(
        ...
        details: {
          ...
          previous_github_run_status:
            githubRunStatus,
          previous_github_run_conclusion:
            githubRunConclusion,
    ...
    return {
      ok: true,
      mode:
        "rollback_retry_authorization_only",
      restore_execution_id:
        restoreExecutionId,
      rollback_status:
        "required",
...
// =========================
// ADMIN SAFE CONTROL RESET AFTER PRE-DESTRUCTIVE ROLLBACK FAILURE
// Stage 2I-SR14F-D5B-2I
//
// This endpoint never starts or retries rollback. It is available only when
// a rollback workflow is proven to have completed unsuccessfully before any
// Main/PII rollback stage started. It changes only the control-state marker
// from failed -> required while deliberately preserving rollback_github_run_id
// and the failure evidence. Preserving the run id, plus the recovery-action
// lock, keeps rollback dispatch prohibited until a separate future recovery
// authorization is explicitly designed.
// =========================
Router.register(
  "POST",
  "/api/admin/restore/rollback/safe-control-reset",
  async (ctx) => {
    ...
    const resetLimit =
      await SecurityRateLimit.consume({
        env: ctx.env,
        scope:
          "admin-restore-rollback-safe-reset",
        key:
          `user:${admin.user_id}`,
        ...ADMIN_RESTORE_ROLLBACK_SAFE_RESET_LIMIT,
      });
    ...
    const expectedConfirmationPhrase =
      `RESET FAILED ROLLBACK CONTROL ${restoreExecutionId}`;
    ...
    if (
      ...
      return {
        error:
          "rollback_safe_reset_confirmation_phrase_incorrect",
        required_confirmation_phrase:
          expectedConfirmationPhrase,
      };
    }
    ...
    if (
      ...
      return {
        error:
          "rollback_safe_reset_not_applicable",
        restore_status:
          journal.status || null,
        rollback_status:
          journal.rollback_status || null,
    ...
    if (
      ...
      return {
        error:
          "rollback_safe_reset_progress_detected",
        manual_review_required:
          true,
        retry_prohibited:
          true,
    ...
    // Fresh GitHub run lookup is mandatory immediately before authorization.
    const githubLookup =
      await getGitHubRollbackRunById(
        ctx.env,
        rollbackGithubRunId
    ...
    if (githubLookup.error) {
      return {
        error:
          "rollback_safe_reset_github_lookup_failed",
        github_error:
          githubLookup.error,
        manual_review_required:
          true,
    ...
    if (
      ...
      return {
        error:
          "rollback_safe_reset_github_run_not_found",
        manual_review_required:
          true,
      };
    }
    ...
    if (
      ...
      return {
        error:
          "rollback_safe_reset_github_failure_not_proven",
        github_run_status:
          githubRunStatus,
        github_run_conclusion:
          githubRunConclusion,
    ...
    // Atomic authorization repeats every safety guard. The GitHub run id is
    // deliberately preserved, so existing dispatch guards continue to block
    // any retry after this control reset.
    const resetResult =
      await ctx.env.OPS_DB.prepare(`
        UPDATE restore_execution_journal
        SET
    ...
    if (
      Number(
        resetResult?.meta?.changes ||
        0
      ) !== 1
    ) {
      return {
        error:
          "rollback_safe_reset_authorization_failed",
        manual_review_required:
          true,
      };
    }
    ...
    await SecurityAudit.recordSafe(
      ctx,
      {
        actorUserId:
          admin.user_id,
        action:
          "admin.restore_rollback_safe_control_reset",
        targetType:
          "restore_execution",
        targetId:
          String(
        ...
        details: {
          ...
          rollback_stage_started:
            false,
          rollback_github_run_id_preserved:
            true,
          destructive_rollback_started_by_worker:
            false,
          retry_started:
    ...
    return {
      ...
      retry_prohibited:
        true,
      future_retry_requires_separate_authorization:
        true,
    };
  }
);
...
Router.register(
  ...
  async (ctx) => {
    ...
    if (
      ...
      return {
        error:
          "rollback_retry_requires_separate_authorization",
        retry_prohibited:
          true,
      };
    }
