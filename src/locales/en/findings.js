const findings = {
  resident: {
    title: "Test Findings",
    subtitle:
      "Report issues found while testing MVX TEST and follow their status.",
    newFinding: "Register a new finding",
    myFindings: "My findings",
    assignedRetests: "Assigned retests",
    noFindings: "You have not registered any findings yet.",
    noRetests: "No retests are assigned to you.",
  },

  form: {
    type: "Finding type",
    title: "Short title",
    reproductionSteps: "Steps to reproduce",
    expectedResult: "Expected result",
    actualResult: "Actual result",
    blocking: "Blocking issue",
    extraExplanation: "Additional explanation",
    route: "Page / route where the issue was observed",
    screenshot: "Screenshot",
    screenshotHint:
      "Optional. PNG, JPEG or WebP, maximum 5 MB.",
    submit: "Register finding",
    submitting: "Registering...",
    requiredHint: "Required fields are marked with *.",
  },

  types: {
    bug: "Bug",
    usability_ux: "Usability / UX",
    text_translation: "Text / translation",
    documentation: "Documentation",
    suggestion: "Suggestion",
  },

  status: {
    NEW: "New",
    NEEDS_INFO: "Needs information",
    HOLD: "On hold",
    APPROVED: "Approved",
    IN_PROGRESS: "In progress",
    READY_FOR_RETEST: "Ready for retest",
    VERIFIED: "Verified",
    CLOSED: "Closed",
  },

  detail: {
    title: "Finding details",
    close: "Close details",
    number: "Number",
    type: "Type",
    status: "Status",
    created: "Created",
    route: "Route",
    blocking: "Blocking",
    yes: "Yes",
    no: "No",
    steps: "Steps to reproduce",
    expected: "Expected result",
    actual: "Actual result",
    explanation: "Additional explanation",
    reason: "Status reason",
    environment: "Environment",
    language: "UI language",
    commit: "Application commit",
    browser: "Browser",
    operatingSystem: "Operating system",
    screen: "Screen",
    screenshot: "Screenshot",
    loadingScreenshot: "Loading screenshot...",
    screenshotUnavailable: "Screenshot could not be loaded.",
    noScreenshot: "No screenshot attached.",
    retryScreenshot: "Attach screenshot",
    uploadScreenshot: "Upload screenshot",
    history: "History",
    noHistory: "No history entries.",
    retestHistory: "Retest history",
    noRetestHistory: "No retest history.",
  },

  info: {
    title: "Additional information requested",
    description:
      "The administrator requested additional information for this finding.",
    placeholder: "Enter the requested information...",
    submit: "Send information",
    sending: "Sending...",
  },

  retest: {
    active: "Retest required",
    completed: "Completed",
    assignedBy: "Assigned by",
    assignedAt: "Assigned",
    comment: "Retest comment",
    commentPlaceholder:
      "Describe what you checked and the result...",
    pass: "PASS",
    fail: "FAIL",
    submitting: "Submitting...",
    result: "Result",
    completedAt: "Completed",
  },

  event: {
    created: "Finding created",
    info_added: "Additional information added",
    info_requested: "Additional information requested",
    status_changed: "Status changed",
    retest_assigned: "Retest assigned",
    retest_pass: "Retest passed",
    retest_fail: "Retest failed",
    implementation_updated: "Implementation reference updated",
  },

  messages: {
    loading: "Loading findings...",
    loadFailed: "Unable to load findings data.",
    operationFailed: "Operation failed",
    created: "Finding {id} was registered.",
    createdWithScreenshot:
      "Finding {id} was registered with a screenshot.",
    createdScreenshotFailed:
      "Finding {id} was registered, but the screenshot was not uploaded. The finding is saved; you can retry while it remains NEW.",
    screenshotUploaded: "Screenshot uploaded.",
    informationSent: "Additional information sent.",
    retestPass: "Retest submitted as PASS.",
    retestFail: "Retest submitted as FAIL.",
    requiredFields: "Complete all required fields.",
    invalidScreenshot:
      "Select a PNG, JPEG or WebP image no larger than 5 MB.",
  },

  testOnly:
    "Available only in the MVX TEST environment.",

  admin: {
    title: "Findings Register",
    subtitle:
      "Review and manage findings submitted during MVX TEST testing.",
    sectionTitle: "Test Findings Register",

    filters: "Filters",
    number: "Finding number",
    tester: "Tester",
    role: "Role",
    route: "Route / page",
    version: "Application commit",
    from: "Created from",
    to: "Created to",
    statusFilter: "Status",
    typeFilter: "Type",
    blockingFilter: "Blocking",
    allStatuses: "All statuses",
    allTypes: "All types",
    allBlocking: "All",
    blockingYes: "Blocking",
    blockingNo: "Not blocking",
    applyFilters: "Apply filters",
    resetFilters: "Reset",

    loading: "Loading findings...",
    noFindings: "No findings match the selected filters.",
    loadFailed: "Unable to load findings register.",
    total: "Total",
    previous: "Previous",
    next: "Next",
    page: "Page",

    columnNumber: "Number",
    columnTitle: "Title",
    columnTester: "Tester",
    columnType: "Type",
    columnStatus: "Status",
    columnBlocking: "Blocking",
    columnCreated: "Created",

    detailTitle: "Finding details",
    closeDetail: "Close details",
    detailFailed: "Unable to load finding details.",
    author: "Tester",
    authorRoles: "Role snapshot",
    authorMode: "Mode",
    created: "Created",
    updated: "Updated",
    blocking: "Blocking",
    yes: "Yes",
    no: "No",
    steps: "Steps to reproduce",
    expected: "Expected result",
    actual: "Actual result",
    explanation: "Additional explanation",
    reason: "Status reason",
    environment: "Environment",
    language: "UI language",
    commit: "Application commit",
    browser: "Browser",
    operatingSystem: "Operating system",
    screen: "Screen",

    implementation: "Implementation references",
    githubIssue: "GitHub issue",
    implementationRef: "Implementation reference",

    screenshot: "Screenshot",
    loadingScreenshot: "Loading screenshot...",
    noScreenshot: "No screenshot attached.",
    screenshotUnavailable: "Screenshot could not be loaded.",

    history: "Event history",
    noHistory: "No history entries.",
    fromStatus: "From",
    toStatus: "To",
    eventReason: "Reason",
    eventComment: "Comment",
    actor: "Actor",

    retests: "Retest history",
    noRetests: "No retests.",
    assignedTo: "Assigned to",
    assignedBy: "Assigned by",
    assignedAt: "Assigned",
    outcome: "Outcome",
    completedAt: "Completed",
    retestComment: "Comment",

    workflow: {
      title: "Administrative decision",
      requestInfo: "Request additional information",
      infoPlaceholder: "Describe what information the tester must provide...",
      infoRequired: "Enter the information request.",
      infoRequested: "Additional information requested.",
      changeStatus: "Change finding status",
      targetStatus: "Target status",
      holdReason: "Reason for HOLD",
      holdReasonRequired: "A HOLD reason is required.",
      closeReason: "Closing reason",
      closeReasonRequired: "Select a closing reason.",
      reasonDetails: "Additional reason details",
      statusComment: "Administrative comment",
      passRetestRequired: "VERIFIED requires the latest completed retest to be PASS.",
      apply: "Apply status change",
      saving: "Saving...",
      statusUpdated: "Finding status updated.",
      noStatusActions: "No further status transitions are available.",
      retestAssignment: {
        title: "Assign retest",
        description:
          "Retest may be assigned only to an active user whose authoritative MVX role is Resident or Owner.",
        loadCandidates: "Load tester candidates",
        loadingCandidates: "Loading testers...",
        tester: "Tester",
        checkingRoles: "Checking authoritative roles...",
        eligible: "Eligible for retest assignment.",
        notEligible: "This user is not eligible for retest assignment.",
        roles: "Roles",
        comment: "Assignment comment",
        commentPlaceholder:
          "Optional instructions for the tester...",
        assign: "Assign retest",
        assigning: "Assigning...",
        assigned: "Retest assigned.",
        active: "An active retest already exists.",
        assignedTo: "Assigned to",
        operationFailed: "Retest assignment failed",
      },

      implementationTitle: "Implementation tracking",
      saveImplementation: "Save implementation references",
      implementationSaved: "Implementation references saved.",
      operationFailed: "Operation failed",
      closeReasons: {
        duplicate: "Duplicate",
        expected_behaviour: "Expected behaviour",
        not_reproduced: "Not reproduced",
        test_data_issue: "Test data issue",
        documentation_issue: "Documentation issue",
        wont_fix: "Will not fix",
      },
    },

    readOnlyNotice:
      "Workflow actions will be enabled in the next implementation stage.",
  },
};

export default findings;
